...
import { useNightStudyProjectSuspense } from "../hooks/useNightStudySuspense";

export const NightStudyProjectList = () => {
  const { colors } = useTheme();
  const navigation = useNavigation<any>();
  const queryClient = useQueryClient();
  const items = useNightStudyProjectSuspense();
  const deletingRef = useRef(false);

  const handleApply = useCallback(() => {
    navigation.navigate("NightStudyApply", { tab: "project" });
  }, [navigation]);
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      ...
      toast.success("프로젝트 심야 자습이 삭제되었어요.", { position: "top" });
    } catch {
    ...
    return (
      <View style={[styles.card, { backgroundColor: colors.background.surface }]}>
        <EmptyState
          ...
          message="아직 신청한 프로젝트 심야 자습이 없어요."
          cta={
